...
class VendorController extends Controller
{
    ...
    public function get_items(Request $request)
    {
      ...
      if($request->type == 'all'){
          $paginator = Item::with('tags')->Approved()->where('store_id', $request['vendor']->stores[0]->id)->latest()->paginate($limit, ['*'], 'page', $offset);
          // return $request['vendor']->stores[0]->id;
      }else{
      $paginator = Item::with('tags')->Approved()->where('store_id', $request['vendor']->stores[0]->id)->where('category_id',$request->type)->latest()->paginate($limit, ['*'], 'page', $offset);
      }
        $data = [
            'total_size' => $paginator->total(),
            'limit' => $limit,
